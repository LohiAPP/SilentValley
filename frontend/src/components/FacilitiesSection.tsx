    ...
    return (
        <section id="facilities" ref={targetRef} className="bg-nature-dark relative">
            ...
            <div className="block md:hidden py-12">
                <div className="container mx-auto px-2 md:px-4 max-w-7xl mb-8">
                ...
                <div className="flex overflow-x-auto snap-x snap-mandatory gap-6 px-6 pb-8 scrollbar-hide">
                    {upcoming.map((facility, index) => (
                        ...
                        >
                            ...
                            />
                            <div className="absolute inset-0 bg-linear-to- from-nature-dark via-transparent to-transparent" />
            ...
            <div className="hidden md:block relative h-[225vh]">
                <div className="sticky top-0 h-screen flex flex-col justify-center overflow-hidden">
                    ...
                    {/* Background Watermark - Pinned to top to prevent vertical cutoff */}
                    <div className="absolute top-8 left-6 md:left-12 w-full z-0 hidden md:block pointer-events-none">
                        <motion.h3
                            initial={{ opacity: 0 }}
                            whileInView={{ opacity: 1 }}
                            transition={{ duration: 1 }}
                            className="font-serif text-nature-light/50 font-bold uppercase tracking-widest whitespace-nowrap text-[clamp(3rem,6vw,6rem)]"
                        >
                            Upcoming Facilities
                        </motion.h3>
                    </div>
                    ...
                    {/* Main Section Header */}
                    <div className="container mx-auto px-2 md:px-4 mb-8 relative z-20 md:pl-10 max-w-7xl">
                        <h3 className="text-5xl md:text-6xl font-serif text-nature-text mb-2 drop-shadow-md">
                    ...
                    <div className="flex h-full items-center">
                        <motion.div style={{ x }} className="flex gap-8 pl-2 md:pl-10">
                            {upcoming.map((facility, index) => (
                                <div
                                    key={index}
                                    className="group relative h-[400px] w-[300px] md:h-[500px] md:w-[400px] shrink-0 rounded-3xl overflow-hidden shadow-2xl border border-nature-light/10 bg-nature-deep"
                                >
                                    {/* Background Image */}
                                    <Image
                                        src={facility.image}
                                    ...
                                    {/* Gradient Overlay */}
                                    <div className="absolute inset-0 bg-linear-to- from-nature-dark via-nature-dark/50 to-transparent" />
                                    ...
                                    <div className="absolute bottom-0 left-0 right-0 p-8 transform translate-y-4 group-hover:translate-y-0 transition-transform duration-500">
                                        ...
                                        <h4 className="text-3xl font-bold text-nature-text mb-3 leading-tight">{facility.title}</h4>
                                        <p className="text-nature-text/80 text-lg leading-relaxed opacity-0 group-hover:opacity-100 transition-opacity duration-500 delay-100">
                                            {facility.desc}
                                        </p>
                                    </div>
                                </div>
