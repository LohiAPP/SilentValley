                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    {attractions.map((item, index) => (
                        ...
                        >
                            ...
                            {/* Gradient Overlay */}
                            <div className="absolute inset-0 bg-linear-to- from-nature-deep/90 via-nature-deep/40 to-transparent opacity-80 group-hover:opacity-90 transition-opacity duration-300" />

                            {/* Content */}
                            <div className="absolute bottom-0 left-0 right-0 p-8 transform translate-y-4 group-hover:translate-y-0 transition-transform duration-300">
                                <h4 className="font-serif text-3xl lg:text-4xl font-bold text-nature-text mb-2">
                                    {item.title}
                                </h4>
                                {item.subtext && (
                                    <span className="text-sm lg:text-base text-nature-light uppercase tracking-wider block mb-2">{item.subtext}</span>
                                )}
                                <p className="text-nature-text/80 text-lg lg:text-xl line-clamp-3 group-hover:line-clamp-none transition-all">
                                    {item.description}
                                </p>
                            </div>
                        </motion.div>
